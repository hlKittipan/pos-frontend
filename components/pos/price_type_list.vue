...
export default {
  ...
  async beforeCreate() {
    ...
    if(pricetype) {
      this.data = this.$store.getters['pos/getPriceTypeList']
  ...
  methods: {
    ...
    async save(id) {
      ...
      console.log(target)
      const response = await this.$store.dispatch('pos/updatePriceType',target)
      if (response) {
    ...
    async onDelete(id) {
      const newData = [...this.data]
      const response = await this.$store.dispatch('pos/deletePriceType',id)
      if (response) {
        this.$notification.open({
