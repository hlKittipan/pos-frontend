...
export default {
  ...
  async beforeCreate() {
    const response = await this.$store.dispatch('pos/fetchPriceTemplate')
    if (response) {
      this.data = this.$store.getters['pos/getPriceTemplateList']
  ...
  computed: {
    ...
    getPriceTemplateList(){
      return this.$store.getters['pos/getPriceTemplateList']
    }
  },
  methods: {
    ...
    async onDelete(id) {
      id = 'fdsfdsf'
      const response = await this.$store.dispatch('pos/deleteProduct',id)
      if (response) {
        this.$notification.open({
