...
export default {
  name: 'AddOnTypeList',
  async beforeCreate() {
    await this.$store.dispatch('pos/fetchAddOnType')
    this.data = this.$store.getters['pos/getAddOnTypeList']
  ...
  methods: {
    ...
    async save(id) {
      ...
      console.log(target)
      const response = await this.$store.dispatch('pos/updateAddOnType',target)
      if (response) {
    ...
    async onDelete(id) {
      const newData = [...this.data]
      const response = await this.$store.dispatch('pos/deleteAddOnType',id)
      if (response) {
        this.$notification.open({
